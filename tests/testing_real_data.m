...
%% ----------Open the file----------%
data_upsample = load('data_2.4295fc_another_rec.mat').data;

data_upsample = data_upsample(1:3e7);
...
%% ----------- basic plots ------------ %
f = -fs_start/2:fs_start/length(data_upsample):fs_start/2 - fs_start/length(data_upsample);
t = 0:1/fs_start:length(data_upsample)/fs_start-1/fs_start;

figure;
plot(t,abs(data_upsample))

figure;
plot(f,abs(fftshift(fft(data_upsample))))
...
[peak,max_sync_time_4] = max(sync_time_4);
...
spectrogram(sync_freq_data,100,[],[],fs,'centered','yaxis')
