...
% find_freq_with_matrix(sym_6,seq_6,fs)
...
sync_freq_data_first_ofdm = sync_data(Ncp_prime+1:nfft+Ncp_prime);
sync_freq_data_last_ofdm = sync_data(nfft*8+Ncp*9+8*2+1:nfft*9+Ncp*9+8*2);

sync_data_middle = reshape(sync_data(nfft+Ncp_prime+1:nfft*8+Ncp*8+8),Ncp+nfft,[]);
...
phase_offset = find_phase_offset(seq_6,sym_6,fs)
...
% 
% function ofdm_demod(sync_data, Ncp,Ncp_prime,zadof)
% 
% 
% 
% 
...
function H_est = estimate_channel(zadof,pilot)
    ...
    H_est = movmean(H,16,'Endpoints','shrink');
end
